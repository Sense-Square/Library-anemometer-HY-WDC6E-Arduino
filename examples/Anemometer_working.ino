...
void loop()
{
    ...
    debugPrint(response, bytesRead);

    short windDirection = get_wind_direction(response);
    float windSpeed = get_wind_speed(response);
    float temperature = get_temperature(response);
    float humidity = get_humidity(response);
    float pressure = get_pressure(response);
